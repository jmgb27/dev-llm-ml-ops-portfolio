(() => {
  ...
  function renderEmptyState() {
    chatEl.innerHTML = `
      <div class="chat__empty">
        ...
        <p>Ask anything — responses stream from your local Llama 3.2 model via the LiteLLM gateway.</p>
      </div>
    `;
  }
  ...
  async function streamChat() {
    ...
    const { wrapper, contentEl } = createMessageElement("assistant", "", { typing: true });
    chatEl.appendChild(wrapper);
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      messages.pop();
      const userMessages = chatEl.querySelectorAll('.message[data-role="user"]');
  ...
  async function loadConfig() {
    try {
      ...
      // Non-fatal — default badge text is fine.
    }
  }
